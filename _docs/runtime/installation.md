...
**Ingress host**  
  * The IP address or host name of the ingress controller component.  
    ...
    * Silent install: Add the `--ingress-host` flag. If a value is not provided, then uses NGINX host from the NGINX ingress controller associated with the **Ingress class**. 
