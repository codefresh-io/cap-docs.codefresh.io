...
* Make sure your ingress controller is configured correctly:
  * [NGINX Enterprise configuration]({{site.baseurl}}/docs/runtime/requirements/#nginx-enterprise)
...
1. Complete the configuration for ingress controllers:
  * [NGINX Enterprise: Post-installation configuration]
...
* Runtime flags, relating to the runtime, cluster and namespace requirements
...
**Skip ingress**  
Optional.  
If you are not using one of the supported ingress controllers, bypass installing ingress resources with the `--skip-ingress` flag.  
