...
* Monitor Activity Log notifications for sensor and event-source errors
* Monitor, analyze, and compare resource consumption for all pipelines in the global aggregated view, or for a single pipeline in the list view.
* Drill down to the workflow for workflow-step visualizations, and retry, resubmit, terminate workflows 
* Update pipeline configurations - Workflow Template, Trigger conditions and arguments 
...
Monitor pipeline activity and analyze the data to derive key insights.  
There are two levels of reporting for pipelines through two dashbaords:
Aggregated  reporting which is a global view analytics for all the Delivery Pipelines with active workflows in your cluster, displayed in the Home page. Use to analyze trends, resource consumption, and .  
Individual pipeline reporting: 


#### View global pipeline reports
...
#### Explore global pipeline view
Here is an example of the global reporting view for pipelines. 
...
The table describes the information in the global pipeline view.
...
#### Filters for global pipeline analytics
Filters narrow the scope of aggregated reporting, allowing you to visualize just the information you want to see. Unless otherwise indicated, all filters support multi-selection.
...
#### Metrics in global pipeline analytics
...
#### View individual pipelines (list view)
